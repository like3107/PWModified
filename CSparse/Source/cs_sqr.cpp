...
static int cs_vcount (const cs *A, css *S)
{
    ...
    S->pinv = pinv = static_cast<int*>(cs_malloc (m+n, sizeof (int)) );        /* allocate pinv, */
    S->leftmost = leftmost = static_cast<int*>(cs_malloc (m, sizeof (int)) );  /* and leftmost */
    w = static_cast<int*>(cs_malloc (m+3*n, sizeof (int)) );   /* get workspace */
    if (!pinv || !w || !leftmost)
    {
...
css *cs_sqr (int order, const cs *A, int qr)
{
    ...
    n = A->n ;
    S = static_cast<css*>(cs_calloc (1, sizeof (css)) );       /* allocate result S */
